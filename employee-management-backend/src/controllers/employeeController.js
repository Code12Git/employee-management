...
const deleteOne = async (request, response) => {
    try {
        const result = await employeeManager.delete(request.params.id);
...
const filter = async (request, response) => {
    try {
        const result = await employeeManager.filter(request.query);
        return responseManager.sendSuccessResponse(response, result, "Filtered employees successfully");
    } catch (err) {
        return responseManager.sendErrorResponse(response, err, "Failed to filter employees");
    }
};
